...
my $usnic_builds = "http://savbu-swucs-bld3.cisco.com/vic/1.5.3/Drivers/Linux/Network/Cisco/1225x";
my $usnic_subdir = "RHEL/RHEL6.4/usNIC";
...
if ($src_image eq "") {
    ...
    print "perl vds-make-usnic-build-swimage.pl source_image usnic_build_id\n";
    exit(1);
}
...
my $build_id = $ARGV[1];
if ($build_id eq "") {
    print "Must specify a usnic build id as argv[2]\n";
    print "perl vds-make-usnic-build-swimage.pl source_image usnic_build_id\n";
    exit(1)
}
...
print "url: $usnic_pkg_dir\n";

my @dirlist;
sub find_latest_usnic_build {
    my $files = shift;

    my $file = "/tmp/usnic.html";
    my $ret = system("wget -q $usnic_pkg_dir -O $file");
    abort "Can't wget usnic image info"
        if (0 != $ret);

    open(IN, $file) || abort "Can't open usnic html file";
    while (<IN>) {
        if (m/<a href="(kmod-usnic_verbs-.*\.rpm)">/ ||
	    m/<a href="(libusnic_verbs-.*\.rpm)">/ ||
	    m/<a href="(openmpi-cisco-.*\.rpm)">/ ||
	    m/<a href="(usnic_tools-.*\.rpm)">/) {

	    my $rpm = $1;
	    if ($rpm !~ /src/) {
	        push(@{$files}, "$usnic_pkg_dir/$rpm");
	    }
        }
    }
    close(IN);
...
find_latest_usnic_build(\@rpms_to_install);
print join("\n", @rpms_to_install);
return;
